...
class WaterElectricityCalculator:
    ...
    def _create_result_display_area(self, parent_frame):
        ...
        # 一楼结果 - 添加明显的标题框架
        floor1_frame = ttk.LabelFrame(frame, text="🏠 一楼费用明细", padding=5)
        floor1_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)

        self.floor1_text = tk.Text(floor1_frame, width=45, height=20)
        self.floor1_text.pack(fill=tk.BOTH, expand=True)
        self.floor1_text.insert(tk.END, "一楼费用明细（含计算过程）:\n\n")
        self.floor1_text.config(state=tk.DISABLED)

        # 二楼结果 - 添加明显的标题框架
        floor2_frame = ttk.LabelFrame(frame, text="🏢 二楼费用明细", padding=5)
        floor2_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5)

        self.floor2_text = tk.Text(floor2_frame, width=45, height=20)
        self.floor2_text.pack(fill=tk.BOTH, expand=True)
